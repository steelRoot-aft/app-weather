...
function App() {
  ...
  const [city, setCity] = useState('Kharkov');

  useEffect(() => {
    async function fetchWeather() {
      setLoading(true);
      const API_KEY = 'd07754a5179043f9a4873910251302';
      const API_URL__CURRENT = `https://api.weatherapi.com/v1/current.json?key=${API_KEY}&q=${city}&lang=ru&aqi=no&alerts=no`;
      const API_URL_FORECAST = `https://api.weatherapi.com/v1/forecast.json?key=${API_KEY}&q=${city}&days=5&lang=ru&aqi=no&alerts=no`;
      try {
        const currentWeatherInfo = await axios.get(API_URL__CURRENT);
        const forecastWeatherInfo = await axios.get(API_URL_FORECAST);
        setCurrentWeather(currentWeatherInfo.data);
        setForecastWeather(forecastWeatherInfo.data);
        setForecastDay(forecastWeatherInfo.data.forecast.forecastday);
      } catch (err) {
        setError('Погода не найдена');
      } finally {
        setLoading(false);
      }
    }
  ...
  // Функция для форматирования даты для main ===========================
  const formatDate = (dateStr) => {
    ...
    return { time, date: dateStrFormatted };
  };

  const { time, date } = currentWeather
    ? formatDate(currentWeather.location.localtime)
    : {};

  // Форматирование время для прогноза на 5 дней ===========================
  const formatForecastDate = (dateStr) => {
    const dateObj = new Date(dateStr);
    const day = dateObj.getDate(); // число месяца
    const weekday = dateObj.toLocaleDateString('ru-RU', { weekday: 'long' }); // день недели
    return `${day} ${weekday}`;
  };

  return (
    <>
      <main className={appStyle.body}>
        <div className={appStyle.main}>
          <div className={appStyle.logo}>
            <a href="#">
              <h2>
                steel<span>Root</span>
              </h2>
            </a>
          </div>
          {currentWeather && (
            <div className={appStyle.current}>
              <h1>{Math.round(currentWeather.current.temp_c)}°</h1>
        ...
        <div className={appStyle.detal}>
          ...
          {currentWeather && (
            <div className={appStyle.about}>
              ...
              <ul className={appStyle.list}>
                ...
                <li>
                  Влажность <span>{currentWeather.current.humidity}%</span>
                  {humadity()}
                </li>
                <li>
                  Облачность <span>{currentWeather.current.cloud}%</span>
                  {cloudy()}
                </li>
                <li>
                  Ветер
                  <span>{Math.round(currentWeather.current.wind_kph)}км/ч</span>
                  {wind()}
                </li>
              </ul>
            </div>
          ...
          <div className={appStyle.forecast}>
            <h2>Прогноз погоды на 5 дней...</h2>
            {forecastDay.map((item, index) => {
              return (
                <div className={appStyle.block} key={index}>
                  ...
                  <div className={appStyle.right}>
                    <p>{Math.floor(item.day.avgtemp_c)}</p>
                  </div>
                </div>
              );
            })}
